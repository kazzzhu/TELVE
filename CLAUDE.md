...
- **`index.html` itself has no cache buster** — it *carries* the `?v=`, it can't bust itself, and GitHub Pages caches it for the same 10 minutes. So a change to the HTML (an attribute, a new element) does not reach you on an ordinary reload even with the bump: you need a hard reload (`Ctrl+Shift+R`) or a 10-minute wait. This has already been mistaken once for a bug in the code.
